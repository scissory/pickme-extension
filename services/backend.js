...
const STRINGS = {
  ...
  addingBT:  'Adding BT for tag:%s on behalf of u:%s',
  btBroadcast: 'Broadcasting battletag %s for u:%s',
  sendBT: 'Sending BT list %s to c:%s',
...
function getBTTextAreaString() {
    return battletagsSubmitted.toString().replace(/,/g, "");
}

function addBattleTagToList(req) {
    ...
    verboseLog(STRINGS.addingBT, req.payload.btName, opaqueUserId);

    battletagsSubmitted.push(req.payload.btName + "\n");

    // Broadcast the color change to all other extension instances on this channel.
    attemptPartyBroadcast(channelId);
    
    return getBTTextAreaString();

}

function btQueryHandler(req) {
    // Verify all requests.
    const payload = verifyAndDecode(req.headers.authorization);
    ...
    verboseLog(STRINGS.sendBT, battletagsSubmitted.toString(), opaqueUserId);
    return getBTTextAreaString();
}


function attemptPartyBroadcast(channelId) {
    // Check the cool-down to determine if it's okay to send now.
    const now = Date.now();
    const cooldown = channelCooldowns[channelId];
    if (!cooldown || cooldown.time < now) {
        // It is.
        sendPartyBroadcast(channelId);
        channelCooldowns[channelId] = { time: now + channelCooldownMs };
    } else if (!cooldown.trigger) {
        // It isn't; schedule a delayed broadcast if we haven't already done so.
        cooldown.trigger = setTimeout(sendPartyBroadcast, now - cooldown.time, channelId);
    }
}

function sendPartyBroadcast(channelId) {
    ...
    const body = JSON.stringify({
        ...
        message: getBTTextAreaString(),
        targets: ['broadcast'],
    });

    // Send the broadcast request to the Twitch API.
    verboseLog(STRINGS.btBroadcast, getBTTextAreaString(), channelId);
...
(async () => {
  // Handle a viewer request to be added to party.
  server.route({
    method: 'POST',
    path: `/party/add`,
    handler: addBattleTagToList,
  });

  // Handle a new viewer request list already in party.
  server.route({
    method: 'GET',
    path: '/party/get',
    handler: btQueryHandler,
  });
